...
fun PropertySelectionSheet(
    ...
    onItemSelected: (PropertyModel) -> Unit,
    onDismiss: () -> Unit,
) {
...
private fun ItemsListContent(
    ...
    LazyColumn(modifier = modifier) {
        items(items) { property ->
            val propertyTitle = stringResource(property.titleResId)
            ListItemCard(
                modifier = Modifier
                    .height(72.dp)
                    .clickable { onItemSelected(property) },
                item = ListItem(
                    lead = property.displayColor?.let {
                        LeadContent.Text(text = "", color = property.displayColor)
                    },
                    content = MainContent(title = propertyTitle)
                )
            )
        }
    }
